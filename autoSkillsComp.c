#pragma config(Sensor, in2,    gyroLeft,       sensorGyro)
#pragma config(Sensor, in7,    gyroRight,      sensorGyro)
#pragma config(Sensor, dgtl1,  encoderMogo,    sensorQuadEncoder)
#pragma config(Sensor, dgtl3,  encoderRight,   sensorQuadEncoder)
#pragma config(Sensor, dgtl5,  encoderLeft,    sensorQuadEncoder)
#pragma config(Motor,  port1,           tipL,         tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port2,           frontR,        tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           midR,          tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port4,           backR,         tmotorVex393TurboSpeed_MC29, openLoop, reversed)
#pragma config(Motor,  port5,           mogoR,         tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port6,           mogoL,         tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port7,           backL,         tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port8,           midL,          tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port9,           frontL,        tmotorVex393TurboSpeed_MC29, openLoop)
#pragma config(Motor,  port10,           tipR,         tmotorVex393_MC29, openLoop, reversed)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

// This code is for the VEX cortex platform
#pragma platform(VEX2)

// Select Download method as "competition"
#pragma competitionControl(Competition)

//Main competition background code...do not modify!
#include "Vex_Competition_Includes.c"

int front = 1;
int back	= -1;
int right = 1;
int left = -1;

int MAX_POWER = 121;
int MIN_POWER = 40;
int INTEGRAL_ACTIVE_ZONE = 100;
int MAX_INTEGRAL = 50;


int g = 0;
int g2 = 0;
int motorEncoderTest = 0;


int mogoTick = 95;

float error = 0;

bool with = true;
bool without = false;
/*/////////////////////////////*/
/*			 Reset Functions			 */
/*/////////////////////////////*/

void resetDrive()
{
	motor[frontL] = 0;
	motor[backL] = 0;
	motor[frontR] = 0;
	motor[backR] = 0;
}

void constantDrive(int spdC)
{
	motor[frontL] = spdC;
	motor[backL] = spdC;
	motor[frontR] = spdC;
	motor[backR] = spdC;
}

void resetDriveEncoder()
{
	SensorValue[encoderLeft] = 0;
	SensorValue[encoderRight] = 0;
}

void resetMogoEncoder()
{
	SensorValue[encoderMogo] = 0;
}

/*/////////////////////////////*/
/*		Basic Drive Functions		 */
/*/////////////////////////////*/

int encoderPID(int power, int side)
{
	float kp = 0.8;

	int master = 0;
	int slave = 0;
	int error = 1;
	int positive = 0;
	int invert = 1;

	master = (SensorValue[encoderRight] >= SensorValue[encoderLeft]) ? SensorValue[encoderRight] : SensorValue[encoderLeft];
	slave = (SensorValue[encoderRight] >= SensorValue[encoderLeft]) ? SensorValue[encoderLeft] : SensorValue[encoderRight];

	error = master - slave;

	if(power > 0)
	{
		positive = 1;
		invert = 1;
	}
	else
	{
		positive = -1;
		invert = -1;
	}

	if(side == right)
	{
		if(SensorValue[encoderRight] >= SensorValue[encoderLeft])
		{
			power = power - (error * kp * invert);
		}
	}
	else if(side == left)
	{
		if(SensorValue[encoderLeft] < SensorValue[encoderRight])
		{
			power = power - (error * kp * invert);
		}
	}

	if(positive == 1)
	{
		if(power < 0)
		{
			power = 0;
		}
	}
	else
	{
		if(power > 0)
		{
			power = 0;
		}
	}

	return power;
}


void turn(float bearing, bool withMogo)
{

	float kp = 1.2;//0.0745
	float ki = 0;
	float kd = 1;//0.6

	if(withMogo){
		kp = 1.7;
		kd = 1.2;
		ki = 0.1;
	}

	float integral = 0;
	float derivative = 0;

	float lastError = 0;
	float power = 0;
	int powerLeft = 0;
	int powerRight = 0;

	float currentBearing = (SensorValue[gyroLeft]/10)/* + (SensorValue[gyroRight]/10)) / 2*/;
	float targetBearing = bearing + currentBearing;
	float error = targetBearing - currentBearing;

	while(error > 2 || error < -2) // while(error != 0) when PID is done or something with a time limit
	{
		currentBearing = (SensorValue[gyroLeft]/10)/* + (SensorValue[gyroRight]/10)) / 2*/;
		error = targetBearing - currentBearing;
		integral = (error > INTEGRAL_ACTIVE_ZONE || error != 0) ? 0 : (integral + error);
		derivative = error - lastError;

		lastError = error;

		power = (error * kp) + (integral * ki) + (derivative * kd);

		if(power > 0)
		{
			if(power > MAX_POWER)
			{
				power = MAX_POWER;
			}
		}

		else if(power < 0)
		{
			if(power < -MAX_POWER)
			{
				power = -MAX_POWER;
			}
		}
		powerLeft = power;
		powerRight = -power;

		motor[frontL] = powerLeft;
		motor[backL] = powerLeft*0.92;
		motor[frontR] = powerRight;
		motor[backR] = powerRight*0.92;

		if(integral > MAX_INTEGRAL)
		{
			integral = MAX_INTEGRAL;
		}
	}
	resetDrive();
}

void move(float targetDistance, int maxPower)
{
	float kp = 0.22;
	float ki = 0;
	float kd = 1;

	/* Variables used to calculate how fast bot is moving*/
	float currentDistance = 0;
	float lastDistance = 0;
	float lastError = 0;

	error = -30;
	float integral = 0;
	float derivative = 0;

	int power = 0;
	int powerLeft = 0;
	int powerRight = 0;

	resetDriveEncoder();

	int maxTime = 50;

	while(error > 20 || error < -20) // while( error != 0) when finished or a time limit
	{
		currentDistance = (SensorValue[encoderRight] + SensorValue[encoderLeft]) / 2;

		error = targetDistance - currentDistance;

		integral = (error > INTEGRAL_ACTIVE_ZONE || error != 0) ? 0 : (integral + error);
		derivative = error - lastError;
		lastError = error;

		power = (error * kp) + (integral * ki) + (derivative * kd);

		if(power > 0)
		{
			if(power > maxPower)
			{
				power = maxPower;
			}
		}
		else if(power < 0)
		{
			if(power < -maxPower)
			{
				power = -maxPower;
			}
		}

		powerLeft = encoderPID(power, left);
		powerRight = encoderPID(power, right);

		motor[frontL] = powerLeft;
		motor[backL] = powerLeft;
		motor[frontR] = powerRight;
		motor[backR] = powerRight;

		if(integral > MAX_INTEGRAL)
		{
			integral = MAX_INTEGRAL;
		}
		wait1Msec(20);

		if(currentDistance == lastDistance && time1[T1] == maxTime)
		{
			break;
		}
		else
		{
			clearTimer(T1);
		}

		lastDistance = currentDistance;
	}

	resetDrive();
}


void mogo(int choice)
{
	resetMogoEncoder();

	//lower mogo intake
	if(choice == front)
	{
		while(abs(SensorValue[encoderMogo]) <= mogoTick)
		{
			motor[mogoL] = 90;

			motor[mogoR] = 90;
		}
		motor[mogoL] = 15;
		motor[mogoR] = -15;
	}
	//raise mogo intake
	else if(choice == back)
	{
		while(abs(SensorValue[encoderMogo]) <= mogoTick)
		{
			motor[mogoL] = -90;
			motor[mogoR] = -90;
		}
		motor[mogoL] = -15;
		motor[mogoR] = -15;
	}
}

/*void lineTurn(int choice)
{
while(SensorValue[lineFront] != whiteLine && SensorValue[lineCenter] != whiteLine && SensorValue[lineBack] != whiteLine)
{
if(SensorValue[lineLeft] == whiteLine)
{
turn(3);
}
else if(SensorValue[lineRight] == whiteLine)
{
turn(-3);
}
}
resetDrive();
}*/

void pre_auton()
{
	bStopTasksBetweenModes = true;

	resetDriveEncoder();
	resetMogoEncoder();

	SensorType[gyroRight] = sensorNone;
	SensorType[gyroLeft] = sensorNone;
	wait1Msec(1000);
	SensorType[gyroRight] = sensorGyro;
	SensorType[gyroLeft] = sensorGyro;
	wait1Msec(1100);

}

task autonomous()
{
	/*
	THINGS TO BE AWARE OF:
	This autonomous starts on the left side of the red team.
	The commented out parts does not currently have any wait1Msec timers so insert them when nessary.
	If something works then test it out again to make sure it is consistant.
	If the PID turns are overshooting then increase derivative and either increase or decrease integral.
	Proportional for pid is already at a good spot but if its still not working then change.
	Make sure that the robot is set up directly facing the base cone and parallel to the wall. (mobile lift should just slide in without turning)
	Take some time to make sure that 90 degrees is accurate to at least 3~6 degrees of error and try not to improvise using 85 to turn 90.
	*/
	mogo(front);
	move(1225, 90);
	wait1Msec(100);
	mogo(back);
	move(-800, 100);
	turn(-45, with);
	move(-800, 121);
	turn(-90, with);
	constantDrive(107);
	wait1Msec(700);
	constantDrive(18);
	mogo(front);
	constantDrive(-25);
	wait1Msec(100);
	mogo(back);//score 20

	constantDrive(25);
	wait1Msec(400);
	move(-400, 100);
	constantDrive(20);
	wait1Msec(300);
	constantDrive(0);
	move(-100, 100);
	turn(90, without);
	move(400, 100);
	turn(90, without);
	mogo(front);
	move(600, 100);
	mogo(back);
	/*move();
	turn();
	move();
	mogo(front);
	move(-);
	mogo(back);//score 30

	turn();
	move();
	turn();
	mogo(front);
	move();
	mogo(back);
	move(-);
	turn();
	move();
	mogo(front);
	move(-);
	mogo(back);//score 40

	turn(); // turn 180
	mogo(front);
	move();
	mogo(back);
	move();
	turn();
	move(-);
	turn();
	move();
	mogo(front);
	constantDrive();
	mogo(back);//score 60

	turn();
	move();
	turn();
	mogo(front);
	move();
	mogo(back);
	move(-);
	turn();
	move();
	mogo(front);
	move(-);
	mogo(back);//score 70

	constantDrive();//realigne the robot with the 10 point bar
	move(-);
	turn();
	constantDrive();//wall crash
	move();
	turn();
	mogo(front);
	move();
	mogo(back);
	move(-);
	turn();
	move(-);
	turn();
	move();
	mogo(front);
	move(-)
	mogo(back);//score 80

	TOTAL: 80
	/*

	idk what this thing is \/

	/*mogo(front);
	move(425, 121);
	move(20, 70);
	mogo(back);
	wait1Msec(100);
	move(-360, 121);
	turn(84);//degree of turn
	move(260, 121);
	turn(84);
	move(265, 121);
	constantDrive(50);
	mogo(front);
	wait1Msec(50);
	mogo(back);
	wait1Msec(150);
	constantDrive(0);
	move(-242, 121);
	move(80, 70);
	move(-80, 70);
	turn(-86);
	move(192, 121);
	turn(-93);
	mogo(front);
	move(265, 121);
	move(20, 70);
	mogo(back);
	move(-300, 121);
	turn(168);
	mogo(front);
	move(80, 121);
	move(-80, 121);
	*/


}

int threshold = 8;

int channel1;
int channel2;
int channel3;
int channel4;

void deadZoneCheck()
{
	channel1 = (abs(vexRT[Ch1]) > threshold) ? vexRT[Ch1] : 0;
	channel2 = (abs(vexRT[Ch2]) > threshold) ? vexRT[Ch2] : 0;
	channel3 = (abs(vexRT[Ch3]) > threshold) ? vexRT[Ch3] : 0;
	channel4 = (abs(vexRT[Ch4]) > threshold) ? vexRT[Ch4] : 0;

	if(channel2 >= 101)
	{
		channel2 = 100;
	}
	else if(channel2 <= -101)
	{
		channel2 = -100;
	}
	if(channel3 >= 101)
	{
		channel3 = 100;
	}
	else if(channel3 <= -101)
	{
		channel3 = -100;
	}
}

task usercontrol()
{
	// User control code here, inside the loop
	while (true)
	{
		if(vexRT[Btn7L] == 1){
			while(SensorValue[encoderRight] < 902)
			{
				int powerRight = encoderPID(25, right);
				int powerLeft = encoderPID(25, left);

				motor[frontL] = powerLeft;
				motor[backL] = powerLeft;

				motor[frontR] = powerRight;
				motor[backR] = powerRight;
			}

		}
		g = SensorValue[gyroLeft];
		g2 = SensorValue[gyroRight];
		motorEncoderTest = SensorValue[encoderMogo];

		motor[test] = 100;

		if(vexRT[Btn7U]	 == 1)
		{
			motor[backL] = motorType[NULL];
			motor[backR] = motorType[NULL];
			motor[frontL] = motorType[NULL];
			motor[frontR] = motorType[NULL];
			wait1Msec(200);
			motor[backL] = motorType[tmotorVex393TurboSpeed_MC29];
			motor[backR] = motorType[tmotorVex393TurboSpeed_MC29];
			motor[frontL] = motorType[tmotorVex393TurboSpeed_MC29];
			motor[frontR] = motorType[tmotorVex393TurboSpeed_MC29];
			wait1Msec(3000);
			resetDrive();
		}
		deadZoneCheck();

		//drive motors
		motor[frontL] = channel3;
		motor[backL] = channel3;
		motor[frontR] = channel2;
		motor[backR] = channel2;

		//mobile lift goes up on button 6U
		if (vexRT[Btn6U] == 1)
		{
			motor[mogoR] = -100;
			motor[mogoL] = -100;

		}
		//mobile lift goes down on button 6D
		else if (vexRT[Btn6D] == 1)
		{
			motor[mogoL] = 100;
			motor[mogoR] = 100;
		}
		else
		{
			motor[mogoL] = 0;
			motor[mogoR] = 0;
		}
		if(SensorValue[encoderMogo] == 95)
		{
			motor[mogoL] = 0;
			motor[mogoR] = 0;
		}

		if(SensorValue[encoderMogo] == 0)
		{
			motor[mogoL] = 0;
			motor[mogoR] = 0;
		}
		
		if(vexRT[Btn8D] == 1)
		{
		motor[tipL] = -23;
		motor[tipR] = -23;
		}
		if (vexRT[Btn5U] == 1)
		{
		motor[tipL] = -80;
		motor[tipR] = -80;
		}
		//tipper goes down on button 5D
		else if (vexRT[Btn5D] == 1)
		{
		motor[tipL] = 80;
		motor[tipR] = 80;
		}
		else
		{
		motor[tipL] = 0;
		motor[tipR] = 0;
		}
		
	}
}

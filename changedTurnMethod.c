...
void turn(float bearing)//assume positive is right and negative is left
{
	...
	if (targetBearing < SensorValue[gyro]) // Left turn
	{
		while (SensorValue[gyro] > targetBearing + TURN_SLOWDOWN)
		{
			motor[frontL] = -100;
			motor[backL] = -100;
			motor[frontR] = 100;
		...
		while (SensorValue[gyro] > targetBearing)
		{
			motor[frontL] = -50;
			motor[backL] = -50;
			motor[frontR] = 50;
